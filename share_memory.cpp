...
/*
 *brief:   写入double类型的数据进缓存
 *param1:  SendData 发送的双精度浮点数
 *return:  返回写入数据的长度
**/
int CShareMemory::Data2Memory(double SendData){
    char s[128];
    sprintf(s, "%f", SendData);
    std::cout<<"s: "<<s<<std::endl;
    u8 *mychar = (u8*)s;
    u32 length = sizeof(mychar);
    PutToShareMem(mychar, length);
}
...
void * CShareMemory::CreateShareMemory(const char *name, u32 size)
{
    ...
    shmid = shmget(ftok(filename, /*'g'*/4321), size, IPC_CREAT|0666);
    if (-1 == shmid)
    {
